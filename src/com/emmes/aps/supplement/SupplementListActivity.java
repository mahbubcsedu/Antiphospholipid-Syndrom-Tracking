 ...
 * Last Update: Aug 19, 2014 3:25:36 PM
...
import com.emmes.aps.medication.AdapterMedicationList;
...
/**
 * The activity SupplementListActivity is responsible for displaying the user
 * supplement list which they have selected as they are taking currently. At the
 ...
 * {@linkplain android.app.ActionBar} enable the user to add the supplement name
 * which they are taking. This activity will display the added supplement as
 * list with checkbox. The user will be able to select supplement by tapping the
 ...
 * @since 1.0
 */
public class SupplementListActivity extends ListActivity
{
    ...
    /** The Constant TAG. */
    private final static String TAG = "SupplementListActivity";

    /** The adapter. */
    AdapterMedicationList adapter;
	...
	adapter = new AdapterMedicationList(this, retrieveUserMedicationListFromDB(), getAlreadySelectedMedicationForToday());
...
// R.id.textView1, data);
	...
	getListView().setMultiChoiceModeListener(new MultiChoiceModeListener() {
		...
		if (nr != 0)
		{
		    Intent parentintent = NavUtils.getParentActivityIntent(SupplementListActivity.this);
		    parentintent.putExtra("medication", true);
		    NavUtils.navigateUpTo(SupplementListActivity.this, parentintent);
		}
	    }
		...
		case R.id.item_delete:
		    ...
		    break;
		case R.id.item_edit:
		    MedicationDetails med = (MedicationDetails) adapter.getItem(adapter.getSingleCurrentCheckPosition());
...
// Toast.LENGTH_LONG).show();
	Log.d(TAG, "onPause called and updated selected Supplement");
	...
	case R.id.item_add:
	    ...
	    startActivityForResult(intent, 0);
	    // startActivity(i);
	    // Intent intentselft = getIntent();
	    // finish();
	    // startActivity(intentselft);
	    return true;
    ...
    /**
     * <p>
     * This method is for collecting selected medication information from the
     * list activity and store to the database.
     * </p>
     * 
     * @param none
     * @throws none
	...
	int selectedcount = 0;
	ArrayList<MedicationDetails> medList = adapter.mMedList;
    ...
    /**
     * This AskOptionToDelete() method is for creating alert dialog for user
     * confirmation of delete item from medication list.
     * 
     * @param none
     * @throws none
     * @since 1.0
     * @author Mahbubur Rahman
     * @return myQuittingDialogBox return {@linkplain AlertDialog} type object
     *         for delete confirmaiton of medication.
     */
    private AlertDialog AskOptionToDelete()
    {
	AlertDialog myQuittingDialogBox = new AlertDialog.Builder(this.context)
	        // set message, title, and icon
	        .setTitle(getApplicationContext().getResources().getString(R.string.listmed_warn_delete_selected_supplement_title))
	        .setMessage(getApplicationContext().getResources().getString(R.string.listmed_warn_delete_selected_supplement_message))
	        .setPositiveButton(getApplicationContext().getResources().getString(R.string.pop_up_delete_positive_text),
					...
					public void onClick(DialogInterface dialog, int whichButton)
		            {
		                ...
		                ContentValues values = new ContentValues();
		                ArrayList<Integer> selectedMedIds = adapter.getSelectedListIds();
		                String selections = "";
